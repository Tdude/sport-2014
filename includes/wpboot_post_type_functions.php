...
/*-----------------------------------------------------------------------------------*/
/*  Validation, save, delete and miscellaneous functions for custom post types
/*-----------------------------------------------------------------------------------*/
...
add_action('wp_ajax_wpboot_delete_file', 'wpboot_delete_file');
function wpboot_delete_file() {
  if (!isset($_POST['data'])) return;
  list($post_id, $key, $attach_id, $src, $nonce) = explode('!', $_POST['data']);
  if (!wp_verify_nonce($nonce, 'wpboot_ajax_delete_file')) {
    _e('Du har inte r&auml;ttigheter att radera denna fil.');
  }
  wp_delete_attachment($attach_id);
  delete_post_meta($post_id, $key, $src);
  _e('Filen har raderats.');
  die();
}
...
/*-----------------------------------------------------------------------------------*/
  ...
  function add_script_upload() {
    ...
    jQuery(document).ready(function($) {
      // add more file
      $(".wpboot-add-file").click(function(){
      ...
      // delete file
      $(".wpboot-delete-file").click(function(){
        var $parent = $(this).parent(),
          data = $(this).attr("rel");
        $.post(ajaxurl, {action: \'wpboot_delete_file\', data: data}, function(response){
          $parent.fadeOut("slow");
          alert(response);
        });
        return false;
  ...
  // Delete all attachments when deleting post
...
/*-----------------------------------------------------------------------------------*/
  ...
  function show() {
    ...
    wp_nonce_field(basename(__FILE__), 'wpboot_meta_box_nonce');
...
/*-----------------------------------------------------------------------------------*/
  ...
  function show_field_text($field, $meta) {
    ...
    echo "<input type='text' name='{$field['id']}' id='{$field['id']}' value='$meta' style='width:97%' />";
    $this->show_field_end($field, $meta);
  }

  function show_field_textarea($field, $meta) {
    $this->show_field_begin($field, $meta);
    echo "<textarea name='{$field['id']}' cols='60' rows='2' style='width:97%'>$meta</textarea>";
    $this->show_field_end($field, $meta);
  }
  ...
  function show_field_file($field, $meta) {
    ...
    if (!empty($meta)) {
      ...
      $nonce = wp_create_nonce('wpboot_ajax_delete_file');

      echo '<div style="margin-bottom: 10px"><strong>' . _('Uppladdade filer') . '</strong></div>';
      echo '<ol>';
      foreach ($attachs as $att) {
        ...
        if (in_array($src, $meta)) {
          echo "<li>" . wp_get_attachment_link($att->ID) . " (<a class='wpboot-delete-file' href='javascript:void(0)' 
                            rel='{$post->ID}!{$field['id']}!{$att->ID}!{$src}!{$nonce}'>Radera</a>)</li>";
        }
      }
      echo '</ol>';
    }

    // SHOW UPLOAD FORMFIELD
    echo "<div style='clear: both'><strong>" . _('Ladda upp nya filer') . "</strong></div>
      <div class='new-files'>
        <div class='file-input'><input type='file' name='{$field['id']}[]' /></div>
        <a class='wpboot-add-file' href='javascript:void(0)'>" . _('L&auml;gg till fler filer') . "</a>
      </div>
    </td>";
  }
  ...
  function show_field_image($field, $meta) {
    ...
    if (!empty($meta)) {
      ...
      echo '<div style="margin-bottom: 10px"><strong>' . _('Uppladdade bilder') . '</strong></div>';
      foreach ($attachs as $att) {
        ...
        if (in_array($src, $meta)) {
          echo "<div style='margin: 0 10px 10px 0; float: left'><img width='150' src='$src' /><br />
              <a class='wpboot-delete-file' href='javascript:void(0)' rel='{$post->ID}!{$field['id']}!{$att->ID}!{$src}!{$nonce}'>
                                Delete
                            </a>
            </div>";
        }
    ...
    // SHOW UPLOAD FORMFIELD
    echo "<div style='clear: both'><strong>" . _('Ladda upp ny bild') . "</strong></div>
      <div class='new-files'>
        <div class='file-input'><input type='file' name='{$field['id']}[]' /></div>
        <a class='wpboot-add-file' href='javascript:void(0)'>" . _('L&auml;gg till bild?') . "</a>
      </div>
    </td>";
  }
  ...
  function show_field_checkbox_list($field, $meta) {
    ...
    $html = array();
    foreach ($field['options'] as $key => $value) {
      $html[] = "<div class='checkbox-icon'><input type='checkbox' name='{$field['id']}[]' value='$key'" . 
            checked(in_array($key, $meta), true, false) . " /><span class='but-info but-$key'>$value</span></div>";
    }
    echo implode('', $html);
    $this->show_field_end($field, $meta);
  }
...
/*-----------------------------------------------------------------------------------*/
  function save($post_id) {
    ...
    || (!check_admin_referer(basename(__FILE__), 'wpboot_meta_box_nonce'))    // verify nonce
    ...
    foreach ($this->_fields as $field) {
      ...
      // validate meta value
      if (class_exists('wpboot_meta_box_validate') && method_exists('wpboot_meta_box_validate', $field['validate_func'])) {
        $new = call_user_func(array('wpboot_meta_box_validate', $field['validate_func']), $new);
      }
...
/*-----------------------------------------------------------------------------------*/
/*  Extending class taxonomy
/*-----------------------------------------------------------------------------------*/



class wpboot_meta_box_taxonomy extends wpboot_meta_box {
    ...
    if (!is_array($meta)) $meta = (array) $meta;
      ...
      if ('checkbox_list' == $options['type']) {
        foreach ($terms as $term) {
          echo "<input type='checkbox' name='{$field['id']}[]' value='$term->slug'" . 
                    checked(in_array($term->slug, $meta), true, false) . " /> $term->name<br />";
        }
      } else {
        echo "<select name='{$field['id']}" . ($field['multiple'] ? "[]' multiple='multiple' style='height:auto'" : "'") . ">";
